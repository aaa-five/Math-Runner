Math Runner 🛣️🚗💨

📌 Overview

Math Runner is a fast-paced educational Unity game designed for elementary-level students. Players control a moving vehicle that progresses forward by correctly solving basic math problems.

The game helps students practice addition, subtraction, multiplication, and division in a fun and interactive way.

🚀 Features

Vehicle-based endless runner-style gameplay

Math problems based on core arithmetic operations:

Addition (+)

Subtraction (-)

Multiplication (×)

Division (÷)

Swipe-based controls (left/right) to select answers

Increasing difficulty as the player progresses

Designed for elementary school learning reinforcement

Built in Unity for mobile and/or PC

🎮 Gameplay

Players control a moving vehicle that continuously advances through a track of math challenges.

How it works:

A math problem appears on screen

Multiple answer choices are presented

Players swipe left or right to choose the correct answer

Correct answers allow the vehicle to continue forward

Incorrect answers may slow progress or end the run (depending on game rules)

The goal is to strengthen quick mental math skills while maintaining momentum in the game.

🛠️ Built With

Unity Engine

C# scripting

Unity UI system

📂 Project Structure (Suggested)

Assets/
  Scripts/
  Scenes/
  Prefabs/
  UI/

📱 How to Play (Mobile Build)

Install the APK (Android build)

Open Math Runner

Swipe left or right to select answers

Solve math problems to keep your vehicle moving forward

🎯 Educational Purpose

This game is designed to help elementary students:

Improve mental math skills

Recognize basic arithmetic operations

Build fast decision-making skills

Learn through interactive gameplay

💡 Future Improvements

More math levels and difficulty scaling

Timed challenges

Score and leaderboard system

Different vehicle skins/upgrades

Classroom/teacher mode

👨‍💻 Team

Developed as a Unity-based educational game project.

📄 License

This project is for educational purposes.

Have fun learning math while driving forward! 🚗➕➖✖️➗
